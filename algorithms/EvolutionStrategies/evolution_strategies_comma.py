...
def search(max_gens, search_space, pop_size, num_children):
    ...
    for gen in range(max_gens):       # main search loop
        ...
        for i in range(num_children):  
            children.append(mutate(population[i%pop_size], search_space))   # vector and strategy for children mutated from their parents' vector and strategy
            children[i]["fitness"] = objective_function(children[i]["vector"])  # evaluate child fitness based on objective function
        parents = sorted(children, key=itemgetter("fitness"))        # sort children by fitness
        population = parents[:pop_size]                       # best (pop_size) children become new parentss
        print(" > gen", gen, "fitness =", population[0]["fitness"])
    best = population[0]
    return best
...
def main():
    ...
    pop_size = 5            # mu, number of parents
    num_children = 35       # lambda, number of children
