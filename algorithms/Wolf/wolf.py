...
def init_population(minmax, s):
    pop = np.array([dict() for x in range(0, s)])
    ...
    for gen in range(max_generations):
        ...
        for wolf in range(len(population)):
            ...
            #Else move towards the best solution in the visual range
                ...
                best_in_range = sorted(cand_within_range, key=lambda i: i['fitness'])[0]
...
def main():
    ...
    step = 0.5
    population = None
    best = search(objective_function, search_space, max_generations, population, wolves, radius, alpha, step, pa)
    for i in best:
        print(i)
